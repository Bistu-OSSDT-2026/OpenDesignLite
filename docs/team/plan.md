...
- 最小 native shell（Rust + system WebView）
...
- 验证 Cursor/Zed/VS Code 第一目标
- 仅 CLI/MCP 转发，不持状态

**M2 交付**：`apps/extensions/adapter-plan.md`、一个 `odl preview` 或 `handoff` 的编辑器 proof
...
| Preview shell spike | Preview Shell | [preview.md](../specs/preview.md) + 可运行窗口 |
...
| Editor 选型 | Editor Adapters | `apps/extensions/adapter-plan.md` |
