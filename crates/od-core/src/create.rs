//! Artifact creation pipeline shared by CLI and MCP.
//!
//! This module owns the file layout, starter template lookup, design stylesheet
...
pub fn run(kind_slug: &str, root: &Path, options: CreateOptions<'_>) -> Result<CreateResult> {
    ...
    let visual_brief = resolve_visual_brief(kind, options.visual_brief, root);
...
mod tests {
    ...
    fn creates_artifact_files() {
        let root = temp_root("core-create");
        let result = run(
            "html",
            &root,
            CreateOptions {
    ...
    fn temp_root(prefix: &str) -> PathBuf {
        let millis = SystemTime::now()
            ...
            .unwrap()
            .as_millis();
        std::env::temp_dir().join(format!("od-{prefix}-{millis}"))
    }
}
